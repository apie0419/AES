...
BYTE AES_xtime[256];
BYTE AES_2P[256],AES_3P[256]; // 與SBox同乘,02,03 
BYTE AES_9R[256],AES_bR[256],AES_dR[256],AES_eR[256];   // 與Iverser_Sbox同乘,09,0b,0d,0e 
...
void AES_SubBytes_Add_MixColumns(BYTE state[]) {
  int i;
  for(i = 0; i < 16; i += 4) {              // SubBytes + MixColumns 能將 兩個步驟合併成 P 2P 3P 三個表格 其中 P即代表Sbox 2P代表Sbox*{02} 3P代表Sbox*{03} 
    BYTE s0 = state[i + 0], s1 = state[i + 1];
    BYTE s2 = state[i + 2], s3 = state[i + 3];
    state[i + 0] = AES_2P[s0] ^ AES_3P[s1] ^ AES_Sbox[s2] ^ AES_Sbox[s3];
    state[i + 1] = AES_Sbox[s0] ^ AES_2P[s1] ^ AES_3P[s2] ^ AES_Sbox[s3];
    state[i + 2] = AES_Sbox[s0] ^ AES_Sbox[s1] ^ AES_2P[s2] ^ AES_3P[s3];
    state[i + 3] = AES_3P[s0] ^ AES_Sbox[s1] ^ AES_Sbox[s2] ^ AES_2P[s3];
  }
}
...
void AES_SubBytes_Add_MixColumns_Inv(BYTE state[]) {  //SubBytesInv + MixColumnsInv 可以化簡成 9R bR dR eR 四個表格，9R = InvSbox * {09} bR = InvSbox * {0b} 以此類推 
  int i;
  for(i = 0; i < 16; i += 4) {  
    BYTE s0 = state[i + 0], s1 = state[i + 1];
    BYTE s2 = state[i + 2], s3 = state[i + 3];
    state[i + 0] = AES_eR[s0] ^ AES_bR[s1] ^ AES_dR[s2] ^ AES_9R[s3];
    state[i + 1] = AES_9R[s0] ^ AES_eR[s1] ^ AES_bR[s2] ^ AES_dR[s3];
    state[i + 2] = AES_dR[s0] ^ AES_9R[s1] ^ AES_eR[s2] ^ AES_bR[s3];
    state[i + 3] = AES_bR[s0] ^ AES_dR[s1] ^ AES_9R[s2] ^ AES_eR[s3];
  }
}
void AES_Init() {
  int i;
  ...
  for(i = 0; i < 256; i++){
    AES_2P[i] = AES_xtime[AES_Sbox[i]];
    AES_3P[i] = AES_xtime[AES_Sbox[i]] ^ AES_Sbox[i];
    AES_9R[i] = AES_xtime[AES_xtime[AES_xtime[AES_Sbox_Inv[i]]]] ^ AES_Sbox_Inv[i];
    AES_bR[i] = AES_xtime[AES_xtime[AES_xtime[AES_Sbox_Inv[i]]]] ^ AES_xtime[AES_Sbox_Inv[i]] ^ AES_Sbox_Inv[i];
    AES_dR[i] = AES_xtime[AES_xtime[AES_xtime[AES_Sbox_Inv[i]]]] ^ AES_xtime[AES_xtime[AES_Sbox_Inv[i]]] ^ AES_Sbox_Inv[i];
    AES_eR[i] = AES_xtime[AES_xtime[AES_xtime[AES_Sbox_Inv[i]]]] ^ AES_xtime[AES_xtime[AES_Sbox_Inv[i]]] ^ AES_xtime[AES_Sbox_Inv[i]];
  }
...
int AES_ExpandKey(BYTE key[], int keyLen) {
  ...
  for(i = Nk; i < ks; i += 4) {   // 原本的key會被複製在前Nk格，因此從第Nk格開始做運算，一次運算單位為一個word，4bytes=1word，因此一次i+4
    ...
    if (i % Nk == 0) {            // 整除的情況，做w[i] = w[i-Nk] ^ SubWord(RotWord(w[i-4])) ^ Rcon[i/Nk]，這裡我們先把 (SubWord(RotWord(w[i-4])) ^ Rcon[i/Nk])求出
      ...
      memcpy(temp, temp2, 4);
      if ((Rcon <<= 1) >= 256)     //溢位情況，要%(x8+x4+x2+x+1)，等於xor 11B(因為Rcon是int可以超過8bits) 
        Rcon ^= 0x11b;
    }
...
void AES_Encrypt(BYTE block[], BYTE key[], int keyLen) {
  ...
  for(i = 16; i < l - 16; i += 16) {
    ...
    AES_SubBytes_Add_MixColumns(block);  //透過同型做法後，再將SubBytes及MixColumns兩個步驟寫成表，兩個步驟合併為一個。
...
void AES_Decrypt(BYTE block[], BYTE key[], int keyLen) {
  int l = keyLen, i;
  BYTE tempkey[keyLen];    // 儲存做過mixcolumns的key 以免下一個block的key值混亂 
  memcpy(tempkey, key, keyLen);
  AES_AddRoundKey(block, &key[l - 16]);
  for(i = l - 32; i >= 16; i -= 16) {
  	AES_ShiftRows(block, AES_ShiftRowTab_Inv);
  	AES_SubBytes_Add_MixColumns_Inv(block);     //透過同型做法後，再將SubBytes及MixColumns兩個步驟寫成表，兩個步驟合併為一個。 
  	AES_MixColumns_Inv(&tempkey[i]);  // 同型做法將addroundkey及mixcolumns對調，本來 先做 block ^ key 再做 mixcolumns，對調後要變成 mixcolumns(block) ^ mixcolumns(key) 
  	AES_AddRoundKey(block, &tempkey[i]);
  }
...
void AES_CBC_Encrypt(BYTE key[], int expandkeyLen, BYTE data[], int total, BYTE IV[]){
  ...
  for (i = 0; i < total; i += AES_BLOCK_SIZE){
    for(j = i; j< i+16; j++)
      data[j] ^= tempIV[j % AES_BLOCK_SIZE];   // 明文XORIV再做加密
...
void AES_XTS_Encrypt(BYTE key[], int expandkeyLen, BYTE data[], int dataLen, BYTE IV[]){
